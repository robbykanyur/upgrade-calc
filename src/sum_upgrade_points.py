...
def sum_upgrade_points(conn):
  ...
  for result in results:
    result_category = result[4]
    cursor.execute("SELECT * FROM categories WHERE category_name=?", (result_category,))
    simple_category = cursor.fetchone()
    if simple_category:
      result_category = simple_category
